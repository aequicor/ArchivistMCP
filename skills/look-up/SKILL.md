...
### Step 2a. Document found

Response:
```json
{"status": "found", "paths": ["/docs/some-library.md"]}
```

Read each file from `paths`. Use the content as context for the current task. Proceed.

### Step 2b. Document created via sampling

Response:
```json
{"status": "created", "path": "/docs/some-library.md"}
```

Read the created file. Use the content as context. Proceed.

### Step 2c. Sampling unavailable — follow agent instructions
...
4. Call `add_document` with the path and content from the instructions
...
You have not used Resilience4j in this project before → trigger LOOKUP.

1. `find_or_create({"query": "Resilience4j retry"})` → `{"status": "created", "path": "/docs/resilience4j-retry.md"}`
2. Read `/docs/resilience4j-retry.md`
...
You are unfamiliar with the spec → trigger LOOKUP.

1. `find_or_create({"query": "error handling convention"})` → `{"status": "found", "paths": ["/docs/error-handling.md"]}`
2. Read `/docs/error-handling.md`
3. Apply the convention

### Example 3. Sampling unavailable

1. `find_or_create({"query": "Arrow Either monad"})` → returns agent instructions
2. `WebSearch("Arrow Kt Either monad kotlin")`
3. `WebFetch("https://arrow-kt.io/learn/typed-errors/either/")` 
4. Fill the template, call `add_document(path="/docs/arrow-either.md", content=...)`
5. Read `/docs/arrow-either.md`, proceed with the task
